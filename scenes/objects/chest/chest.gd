...
var corn_harvest = preload("res://scenes/objects/plants/corn_harvest.tscn")
...
func _ready():
	...
	feed_component.food_received.connect(on_feed_received)
...
func trigger_feed_harvest(inventory_item: String, scene: Resource):
	...
	for index in inventory_item_count:
		...
		feed_component._on_area_enabled(feed_component)

func on_feed_received(area: Area2D):
